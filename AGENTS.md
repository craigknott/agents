...
usage. Use even when you think you know the answer -- training data may not reflect recent changes. Prefer this over web
...
2. Pick the best match by exact name match, description relevance, code snippet count, source reputation
   (High/Medium preferred), and benchmark score (higher is better). If results do not look right, try alternate names or
   queries, such as `next.js` instead of `nextjs`. Use version-specific IDs when the user mentions a version.
3. Use `query-docs` with the selected library ID and the user's full question, not single keywords.
