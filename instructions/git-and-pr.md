...
- Honor repository commit policies in every commit, including commits generated by automated fix or delivery steps.
  ...
  the author. Automated phases that cannot guarantee that trailer must leave changes uncommitted for the owning
  delivery workflow. Before publishing, and again after each automated commit, verify the complete pull-request commit
  range.
